...
class GraphTransformer(nn.Module):
    def __init__(self, in_size=78, num_class=2, d_model=128, num_heads=8,
        ...
        self.abs_pe = abs_pe#false
        self.abs_pe_dim = abs_pe_dim#0
        ...
        # if in_embed:#它为false，不用看，直接看后面的else代码
        #     if isinstance(in_size, int):
        #         self.embedding = nn.Embedding(in_size, d_model)#（词典长度，向量维度）
        ...
        encoder_layer = TransformerEncoderLayer(
            ...
            gnn_type=gnn_type, se=se, **kwargs)#(128,8,512,0.0,true,gcn,gnn,none)
        ...
        # 512，32，128
        self.cnn_attn = TransformerEncoder(d_model=128, n_head=8, nlayers=3)##encoder部分的层数为3层，embedding维度为128，多头注意力为8个头
        # 512，32，128
        # Q,K,V
    ...
    def forward(self, data, return_attn=False):
        ...
        target = data.target
        #(8,1000)
        node_depth = data.node_depth if hasattr(data, "node_depth") else None#这句话没执行
        ...
        complete_edge_index = data.complete_edge_index if hasattr(data, 'complete_edge_index') else None#none
        abs_pe = data.abs_pe if hasattr(data, 'abs_pe') else None#none
        degree = data.degree if hasattr(data, 'degree') else None#none

        # x = x.long()
        output = self.embedding(x) if node_depth is None else self.embedding(x, node_depth.view(-1, ))
        #(246,128)

        if self.abs_pe and abs_pe is not None:#不走
            abs_pe = self.embedding_abs_pe(abs_pe)
            output = output + abs_pe
        if self.use_edge_attr and edge_attr is not None:
            edge_attr = self.embedding_edge(edge_attr)
            if subgraph_edge_attr is not None:#不走
                subgraph_edge_attr = self.embedding_edge(subgraph_edge_attr)
        else:#走这个
            edge_attr = None
            subgraph_edge_attr = None

        if self.global_pool == 'cls' and self.use_global_pool:#不走这个
            bsz = len(data.ptr) - 1
            if complete_edge_index is not None:#不走
            ...
            if subgraph_node_index is not None:#不走
                idx_tmp = torch.arange(data.num_nodes, data.num_nodes + bsz).to(data.batch)
        ...
        output = self.encoder(
            ...
            ptr=data.ptr,
            return_attn=return_attn
        )#（（246，128），（2，544），none,none,none,none,none,none,none,(9),false）
        # readout step
        if self.use_global_pool:#true
            if self.global_pool == 'cls':
                output = output[-bsz:]
            else:#走这个
                output = self.pooling(output, data.batch)#对节点表征求平均
        if self.max_seq_len is not None:#不走
            pred_list = []
            for i in range(self.max_seq_len):
                pred_list.append(self.classifier[i](output))
            return pred_list
        # return self.classifier(output)
        # sim_common=self.smi_mlp(output)

        # protein--forward#out_dim表示向量的维度，
        ...
        # concat
        xc = torch.cat((output, protein), 1)#8*128，8*128
#8*256
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):#(protein,protein,protein,atten_mask=None)
        ...
        query, key, value = [l(x).view(batch_size, -1, self.h, self.d_k).transpose(1, 2)
                             for l, x in zip(self.linear_layers, (query, key, value))]#（k：8，6，32，16；q：8，8，32，16；v：8，8，32，16）
...
class aTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, atten_mask):#(protein,none)
        ...
        """
        # add & norm 1
        attn = self.dropout(self.self_attn(x, x, x, attn_mask=atten_mask))#(protein,protein,protein,none)#8,32,128
