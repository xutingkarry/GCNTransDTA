...
class Attention(gnn.MessagePassing):
    ...
    def __init__(self, embed_dim, num_heads=8, dropout=0., bias=False,
        symmetric=False, gnn_type="gcn", se="gnn", k_hop=2, **kwargs):#（128，8，0.1，false,gcn,gnn,2,none）
        ...
        if self.se == "khopgnn":
            ...
            self.structure_extractor = StructureExtractor(embed_dim, gnn_type=gnn_type,
                                                          num_layers=k_hop, **kwargs)#128,gcn,2,none
        ...
        self.to_v = nn.Linear(embed_dim, embed_dim, bias=bias)#128,128
    ...
    def forward(self,
            ...
            edge_attr=None,
            ptr=None,
            return_attn=False):#（260，128），（2，586），none,none,none.none,none,none,none,false
        """
        ...
        if self.se == 'khopgnn': # k-subgraph SAT
            ...
            x_struct = self.structure_extractor(x, edge_index, edge_attr)#（260，128），（2，586），none
...
class TransformerEncoderLayer(nn.TransformerEncoderLayer):#跳到这个方法#(128,8,512,0.0,true,graph,gnn,none)
    ...
    def __init__(self, d_model, nhead=8, dim_feedforward=512, dropout=0.1,
                ...
                gnn_type="gcn", se="gnn", k_hop=2, **kwargs):##(128,8,512,0.0,true,graph,gnn,none)
        super().__init__(d_model, nhead, dim_feedforward, dropout, activation)

        self.self_attn = Attention(d_model, nhead, dropout=dropout,
            bias=False, gnn_type=gnn_type, se=se, k_hop=k_hop, **kwargs)#（128，8，0.1，false,gcn,gnn,2,none）
        self.batch_norm = batch_norm#true
        self.pre_norm = pre_norm#false
        if batch_norm:
    ...
    def forward(self, x, edge_index, complete_edge_index,
            ...
            return_attn=False,
        ):#（（260，128），（2，586），none,none,none,none,none,none,none,none,false）

        if self.pre_norm:
            x = self.norm1(x)#260，128--》

        x2, attn, adj = self.self_attn(
            x,
            edge_index,
            complete_edge_index,
            edge_attr=edge_attr,
            ...
            ptr=ptr,
            return_attn=return_attn
        )#（260，128），（2，586），none,none,none.none,none,none,none,false

        if degree is not None:#不走
            x2 = degree.unsqueeze(-1) * x2
        x = x + self.dropout1(x2)
        # x = adj + self.dropout1(x2)
        if self.pre_norm:#走
            x = self.norm2(x)
        else:
            x = self.norm1(x)
        x2 = self.linear2(self.dropout(self.activation(self.linear1(x))))#线性操作，激活函数，防止拟合，线性操作
        x = x + self.dropout2(x2)#残差连接
