...
def get_simple_gnn_layer(gnn_type, embed_dim, **kwargs):#gcn,128,none
    edge_dim = kwargs.get('edge_dim', None)
    if gnn_type == "graph":
        return gnn.GraphConv(embed_dim, embed_dim)
    elif gnn_type == "graphsage":
        return gnn.SAGEConv(embed_dim, embed_dim)
    elif gnn_type == "gatgcn":
        gnn.GATConv(embed_dim,embed_dim)
        nn.ReLU()
        return gnn.GCNConv(embed_dim,embed_dim)
    elif gnn_type == "gcn":#走这个
        if edge_dim is None:
            gnn.GCNConv(embed_dim, embed_dim)#128，128
            gnn.GCNConv(embed_dim, embed_dim)#128，128
            return gnn.GCNConv(embed_dim, embed_dim)#128，128
        else:
            GCNConv(embed_dim, edge_dim)
            GCNConv(embed_dim, edge_dim)
